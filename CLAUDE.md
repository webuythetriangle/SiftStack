...
- **Output suppression (fixed 2026-07-22):** a merged notice is only re-surfaced in that day's output/upload CSV if something in `_MATERIAL_FIELDS` actually changed (`auction_date`, `owner_name`, `tax_delinquent_amount`, `tax_delinquent_years`, `mls_last_sold_date`, `mls_last_sold_price`, `decedent_name`, `decision_maker_name`/`_relationship`, `date_of_death`, `owner_deceased`). AVM/enrichment fields (`estimated_value`, `rvm_*`, etc.) are deliberately excluded since they're re-fetched every run and drift slightly on their own — including them would make every re-notice look "changed" and defeat suppression. A pure re-notice with no material change is dropped from output entirely but still updates the registry's stored data. Before this fix, the registry only merged fields and never suppressed — an unchanged re-notice (new source_url/ID, same facts) still went into the day's CSV and got uploaded to DataSift again, which is what caused two consecutive `nc-daily` runs to upload the same property twice (`4706 Lawndale Dr` / `2110 Hunter St`, Guilford, 2026-07-21 → 2026-07-22).
- **Wired into `enrichment_pipeline.run_enrichment_pipeline()`** as Step 9a, after all enrichment steps run (so the merge sees each notice's freshly-fetched data) and before final validation — every entry point (`daily`, `nc-daily`, `historical`, PDF import, photo import, CSV import) gets this automatically since they all funnel through the shared pipeline. `reconcile_with_registry()` returns `(output_notices, all_merged, updated_count, suppressed_count)` — `output_notices` (filtered) is what continues through the pipeline to CSV/upload; `all_merged` (unfiltered) is what's passed to `update_registry()` so the registry still reflects the latest data even for suppressed repeats.
- **Pre-enrichment short-circuit (added 2026-07-22) — Step 6b, `precheck_unchanged()`:** Step 9a alone still means an unchanged re-notice burns a full Zillow lookup, a NARRPR RVM lookup, and an obituary search before being suppressed — wasteful, and for NARRPR actively harmful (its single-session account gets signed out of the user's own concurrent session for zero new data, see "NARRPR (RPR) RVM Enrichment"). Step 6b runs right after Smarty standardization (Step 6 — the registry key needs a Smarty-standardized address to reliably match a prior run's entry) and compares only the fields parsable straight off the notice text — `auction_date`, `owner_name`, `decedent_name` (`_PRE_ENRICHMENT_FIELDS`, a subset of `_MATERIAL_FIELDS`) — against the registry. An exact match pulls the notice out of the list before Steps 7/8/8b/9 (geocode retry/Zillow/NARRPR/obituary) run at all, already merged with its full prior record, and rejoins the list right before Step 9a — which still runs its own authoritative merge/suppression check on everything, so correctness never depends on Step 6b. Entity research (Step 3a) and tax delinquency (Step 5) run *before* Smarty and are NOT covered by this short-circuit — reordering the pipeline to cover those too was judged too risky for the size of that win (both are low/no-cost: entity research is opt-in and off by default, tax delinquency is mostly cached bulk files/free ArcGIS calls).
...
## NC Foreclosure Notice Scraping — Tax Foreclosure Capture & Search Coverage (2026-07-23)

Two gaps found and fixed in `ncnotices_scraper.py` while validating Durham-county data against a manual site search, both applying automatically to all 5 counties in `NC_SAVED_SEARCHES` (Wake, Durham, Orange, Guilford, Mecklenburg) and to `nc-daily`/`nc-historical` — no CLI or `daily.yml` changes needed since they all funnel through the same shared search code.

**Tax foreclosure notices are now captured, not discarded.** A judicial "in rem"/in-personam tax foreclosure (county/city suing a delinquent-tax owner for a court Judgment, e.g. `"County of Durham and City of Durham vs. Estate of X and Heirs"`) is a legally distinct process from the power-of-sale trustee foreclosure `foreclosure_filter.is_valid_foreclosure()` targets — no deed of trust, no trustee. These used to be silently dropped as "not a real foreclosure." `foreclosure_filter.is_tax_foreclosure()` now classifies them (checked only against notices that already failed the trustee-sale check, so a genuine trustee sale can never be misclassified) and `ncnotices_scraper.py` keeps them as `notice_type="tax_foreclosure"` instead of discarding — no second search needed, since ncnotices.com's single `"foreclosure"`-family keyword search already returns both kinds mixed together. DataSift mapping added: `"tax_foreclosure"` → `"Tax Foreclosure"` list, sale date → `Tax Auction Date` (same field as `tax_sale`, not `Foreclosure Date`).

Getting these to survive the full pipeline required two more fixes, since tax foreclosure notices use a different text format than trustee-sale ones (both hard-won live against Durham, 2026-07-23):
- **Address parsing:** no `"commonly known as X"` phrasing at all — the property is introduced by the legal description itself (`"...more particularly described as follows: 1823 Nixon Street Durham County Tax Parcel 132849..."`). `nc_notice_parser.NC_TAX_FORECLOSURE_ADDR_RE` catches this. City/zip never follow inline either — a fallback (`_NC_SALE_LOCATION_CITY_RE`) pulls the auction's stated city (`"...in Durham, North Carolina..."`, always present per NCGS-mandated notice boilerplate) as a Smarty disambiguation hint, since Smarty's own USPS lookup corrects the city if the street only exists elsewhere in the county.
- **Smarty geo lookup:** `address_standardizer.py` used to combine city/state/zip into one free-text `lastline` string; when city was blank, that collapsed to a bare `"NC"` with no comma, which Smarty's parser sometimes echoed back as if `"NC"` were the city name — blocking zip resolution entirely. Fixed by passing city/state/zip as separate structured fields on the Smarty `Lookup` object instead (`_apply_geo_fields()`), which has no such ambiguity.
- **Known remaining gap:** one address format still fails final validation (missing zip) when the sale-location-city fallback phrase isn't present in that notice's text — accepted as an edge case rather than chased further.

**Search keyword broadened from `"foreclosure"` to `"foreclosure trustee"` (Any Words matching).** Confirmed live that some genuine trustee sale notices are titled `"NOTICE OF SALE"` or `"NOTICE OF TRUSTEE'S SALE OF REAL PROPERTY"` (the latter an HOA assessment-lien foreclosure under NCGS 47F-3-116, not a mortgage deed of trust) and never use the literal word "foreclosure" anywhere in the text — a `"foreclosure"`-only search silently missed them entirely (found by diffing a broad `"foreclosure real+estate"` manual site search against the scraper's own results — see notice IDs 835815/832974 and 788033/785039). Every genuine trustee sale necessarily mentions "trustee" (that's who executes it), so OR-ing the two keywords catches both phrasings. `ncnotices_scraper._run_nc_search_once` selects the site's "Any Words" radio (`NC_SEL_MATCH_ANY_WORDS_LABEL`) automatically whenever a search keyword contains more than one word — the site's default is "All Words" (AND matching), which would have been wrong for a multi-word OR search.

**Scraper resilience: stalled mid-scan restarts now resume from the correct page instead of page 1.** A transient network blip (DNS resolution failure reaching 2captcha.com, or a Turnstile timeout) can leave the results-page DOM stale mid-scan. This used to be silently swallowed — `_scrape_nc_results_page`'s per-result loop treated "fewer view buttons than expected" as ordinary end-of-results and just stopped, with no error, making a stalled page look like a clean, complete run (confirmed: a Durham 12-month run examined only 19 of 82 available notices this way). Now detected loudly and signaled as a crash, and `run_nc_search` restarts the whole search (unavoidable — ASP.NET ViewState is lost on a stall) but fast-forwards straight to the crashed-on page via cheap "next page" clicks rather than re-processing every already-seen notice on earlier pages one at a time — the first version of this fix re-walked page 1 from scratch on every restart, which made recovery itself the slow part on a multi-page crash (a full run took nearly an hour before this was fixed). `seen_ids` still guards against re-adding a notice if a fast-forward ever lands off by one page. Capped at `config.NC_SEARCH_RESTART_LIMIT` (3) attempts before giving up with whatever was collected.
